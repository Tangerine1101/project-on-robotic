...
class PlannerNode(Node):
    def __init__(self):
        ...
        self.get_logger().info(f"Planner are Online. Mode: {mode_str}")
        ...
        try:
            ...
            if self.state == State.WAIT_DEPS:
                if not self._grip_client.service_is_ready():
                    self.get_logger().warn("Waiting for node Driver...", throttle_duration_sec=2)
                else:
                    ...
                    if wait_vis and (not self.vision_active or is_stale):
                        self.get_logger().warn("Waiting for node Vision...", throttle_duration_sec=2)
                    else:
            ...
            # 2. CALIBRATION
                ...
                if not self.calibration_sent:
                    ...
                    if result.success:
                        ...
                        self.get_logger().error("Calibration serivce Failed! Retrying...")
                        self.calibration_sent = False 
                ...
                if not self.latest_objects:
                    ...
                    if not candidates:
                        ...
                        if angles is not None:
                            ...
                            self.get_logger().warn("Unreachable.")
                            self.latest_objects = []
