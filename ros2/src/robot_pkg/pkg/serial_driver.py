...
class SerialDriver(Node):
    def __init__(self):
        ...
        self.PKG_FORMAT = '<Bc b 5f B'  # StartByte, CmdID, Bitmask, 5x Float Args, Checksum
    ...
    # --- BINARY PACKING ---
    def send_binary_pkg(self, cmd_id: str, args: list, bitmask: int = 0x1F):
        """
        ...
        # Checksum Logic
        # We pack with 'b' (signed char) for bitmask to match your C++ struct
        raw_data = struct.pack('<Bc b 5f', self.START_BYTE, cmd_id.encode(), int(safe_bitmask), *full_args)
    ...
    def grip_handle_callback(self, request, response):
        ...
        if request.command in cmd_map:
            ...
            response.message = f"Sent {request.command}"
        else:
    ...
    def read_serial_data(self):
        ...
        try:
            if self.serial_conn.in_waiting > 0:
                # [TSUNDERE FIX]: Check Byte Logic (0x40 / '@')
                # We peek one byte to see if it's the start byte
                # This prevents reading the middle of a stream
                
                # Option A: Simple readline (if you trust the stream)
                # Option B: Sync byte search (Better)
                
                # Let's verify the first byte is '@' (0x40)
                # Note: This assumes your Arduino sends "@10.0,20.0,..."
